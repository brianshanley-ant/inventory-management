...
export default {
  ...
  setup() {
    ...
    const getChangeColor = (forecast) => {
      ...
      if (changePercent <= 2) {
        return '#2a78d6' // mirrors --accent (stable)
      }

      if (change > 0) return '#009300' // mirrors --success (increasing)
      if (change < 0) return '#d03b3b' // mirrors --danger (decreasing)
      return '#2a78d6' // mirrors --accent (no change)
    }
